...
class PerspectiveSeeder extends Seeder
{
    ...
    {
        Log::info("Game perspectives loading started!");

        $snowflake = app('Kra8\Snowflake\Snowflake');

        // Open the file
        if (($handle = fopen(storage_path('app/private/datasets/player_perspectives.csv'), 'r')) !== false) {
            // Skip the header row
            fgetcsv($handle);
            ...
            Log::info("Game perspectives loaded successfully!");
        }
    }

    private function insertBatch($records)
    {
        DB::table('perspectives')->insert($records);
    }
}
