...
class ThemeSeeder extends Seeder
{
    ...
    {
        Log::info("Game themes loading started!");

        $snowflake = app('Kra8\Snowflake\Snowflake');

        // Open the file
        if (($handle = fopen(storage_path('app/private/datasets/themes.csv'), 'r')) !== false) {
            // Skip the header row
            fgetcsv($handle);
            ...
            Log::info("Game themes loaded successfully!");
        }
    }

    private function insertBatch($records)
    {
        DB::table('themes')->insert($records);
    }
}
